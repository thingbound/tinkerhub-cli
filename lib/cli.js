    ...
    withThings(things, () => {
        ...
        if(args[1] === 'metadata') {
            if(args[2] === 'tag') {
				...
				return next(true);
			} else if(args[2] === 'events') {
				for(const thing of things) {
                    out.info(chalk.bgWhite.black(' EVENTS '), thing);
					out.group();
					try {
						out.info(thing.metadata.events);
					} finally {
						out.groupEnd();
					}
                }
				...
				for(const thing of things) {
                    out.info(chalk.bgWhite.black(' STATE '), thing);
					out.group();
					try {
						out.info(thing.metadata.state);
					} finally {
						out.groupEnd();
					}
                }
